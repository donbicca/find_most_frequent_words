...
then 
    ...
    echo 'You can filter out a list of words not needed as result'.
    echo ''
    echo 'find_most_frequent input_file N [''word1''|''word2'']'
    echo
    exit -1
elif [ $# == 3 ]
then
...
echo 'Find the most' $quantity 'frequent words for the input file' $filename 
...
then
    ...
    grep -Ev "^[[:space:]]*$|$filter" < $tmp2 > $tmp1
else 
    echo 'Removing blank lines'
    grep -v "^[[:space:]]*$" < $tmp2 > $tmp1  
fi
